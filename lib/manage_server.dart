...
class _ManageServerPageState extends State<ManageServerPage> with TickerProviderStateMixin {
  ...
  Future<void> _fetchVpsList() async {
    ...
    final uri = Uri.parse('https://tapops.fanzhosting.my.id/myServer?key=${widget.keyToken}');
    try {
  ...
  Future<void> _addVps() async {
    ...
    final uri = Uri.parse('https://tapops.fanzhosting.my.id/addServer');
    try {
  ...
  Future<void> _deleteVps(String host) async {
    ...
    final uri = Uri.parse('https://tapops.fanzhosting.my.id/delServer');
    try {
      final res = await http.post(uri, body: {
        'key': widget.keyToken,
        'host': host,
      ...
      builder: (_) => AlertDialog(
        ...
        title: Row(
          children: [
            ...
            Text(
              title,
              style: TextStyle(color: Color(0xFFfffffe), fontWeight: FontWeight.bold),
            ),
          ],
        ),
        content: Text(
          message,
          style: TextStyle(color: Color(0xFFfffffe).withOpacity(0.8)),
        ),
        actions: [
          TextButton(
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            Container(
                              padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                              decoration: BoxDecoration(
                                color: Color(0xFFfffffe).withOpacity(0.2),
                                borderRadius: BorderRadius.circular(20),
                              ),
                              child: Text(
